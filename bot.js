...
client.on('ready', () => {
client.user.setActivity(`OG.help | ${client.guilds.size} Guilds | ${client.users.size} Users`, {url: "https://twitch.tv/monstercat", type: "STREAMING"});
});
...
client.on('message', async message => { 
	...
	if (message.content.startsWith("OG.") && message.channel.type !== "text")return message.reply("Please use my commands in a server");							
 ...
 if(commandfile){ 
   commandfile.run(client,message,args)
        ...
        if(cmd === `${prefix}play`) {
            ...
            if (voice === undefined) return message.channel.send(`You must be in a voiceChannel in order to play music`);
        ...
        if(cmd === `${prefix}ascii`){
            ...
            if (!args) return message.channel.send('**Proper Usage: `OG.ascii` <Word(s)> **');
